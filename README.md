...
- A stack `--gaming` habilita `i386` automaticamente quando necessário.
- Quando faltarem componentes Debian para jogos, a stack `--gaming` cria `/etc/apt/sources.list.d/debian-bootstrap-gaming.list` com `contrib`, `non-free` e `non-free-firmware`.
- Após ajustar pré-requisitos de jogos, o script atualiza os índices do APT automaticamente.
- Após instalar `steam-installer`, o script tenta iniciar o Steam automaticamente no usuário normal (inclusive quando executado como `root` sem `sudo`, se detectar usuário válido em sessão gráfica).
