...
gaming_components_configured() {
  local need_contrib=true
  local need_non_free=true
  local need_non_free_firmware=true
  local file line repo_line

  for file in /etc/apt/sources.list /etc/apt/sources.list.d/*.list; do
    [ -f "$file" ] || continue
    while IFS= read -r line; do
      repo_line="${line%%#*}"
      if ! printf '%s\n' "$repo_line" | grep -Eq '^[[:space:]]*deb(-src)?[[:space:]].*(debian\.org|debian-security)'; then
        continue
      fi
      if [ "$need_contrib" = true ] && printf '%s\n' "$repo_line" | grep -Eq '(^|[[:space:]])contrib([[:space:]]|$)'; then
        need_contrib=false
      fi
      if [ "$need_non_free" = true ] && printf '%s\n' "$repo_line" | grep -Eq '(^|[[:space:]])non-free([[:space:]]|$)'; then
        need_non_free=false
      fi
      if [ "$need_non_free_firmware" = true ] && printf '%s\n' "$repo_line" | grep -Eq '(^|[[:space:]])non-free-firmware([[:space:]]|$)'; then
        need_non_free_firmware=false
      fi
    done <"$file"
  done
      ...
      while IFS= read -r line; do
        ...
        awk '
          BEGIN { RS=""; FS="\n" }
          {
            ...
            if (uris ~ /(debian\.org|debian-security)/) {
              if (components ~ /(^|[[:space:]])contrib([[:space:]]|$)/) print "contrib"
              if (components ~ /(^|[[:space:]])non-free([[:space:]]|$)/) print "non-free"
              if (components ~ /(^|[[:space:]])non-free-firmware([[:space:]]|$)/) print "non-free-firmware"
            }
          }
        ' "$file"
      )
    done < <(find /etc/apt/sources.list.d -maxdepth 1 -type f -name '*.sources')
  fi

  if [ "$need_contrib" = true ] || [ "$need_non_free" = true ] || [ "$need_non_free_firmware" = true ]; then
    return 1
  fi
}

ensure_gaming_repo_components() {
  if gaming_components_configured; then
    return
  fi
  ...
  if [ -z "$codename" ]; then
    warn "Não foi possível detectar o codename do sistema para habilitar componentes de jogos automaticamente."
    warn "Garanta que os repositórios Debian incluam: contrib non-free non-free-firmware"
    return
  fi

  local managed_source="/etc/apt/sources.list.d/debian-bootstrap-gaming.list"
  if [ "$DRY_RUN" = true ]; then
    warn "Dry-run: stack gaming criaria '$managed_source' com componentes contrib/non-free/non-free-firmware."
    return
  fi

  log "Habilitando componentes Debian para jogos/Steam (contrib, non-free, non-free-firmware)..."
  as_root tee "$managed_source" >/dev/null <<EOF
...
EOF
  ...
  success "Componentes de repositório para jogos habilitados em '$managed_source'."
}
