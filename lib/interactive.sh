...
ensure_any_module_selected() {
  if ! $INSTALL_BASE && ! $INSTALL_TERMINAL && ! $INSTALL_DEV && ! $INSTALL_NETWORK && ! $INSTALL_AUTOMATION && ! $INSTALL_EMBEDDED && ! $INSTALL_GAMING && ! $INSTALL_OPTIONAL && ! $INSTALL_DESKTOP_BASIC && ! $INSTALL_DESKTOP_FULL && [ "$PROFILE" = "none" ]; then
...
interactive_select_modules() {
  ...
  local base_state terminal_state dev_state network_state automation_state embedded_state gaming_state optional_state desktop_state desktop_full_state
  ...
  gaming_state="$(bool_to_onoff "$INSTALL_GAMING")"
  ...
  if [ "$INTERACTIVE_MODE" != "cli" ] && command -v whiptail >/dev/null 2>&1; then
    ...
    choices="$(
      whiptail --title "Debian Bootstrap" \
        ...
        "GAMING" "Stack completa para jogos" "$gaming_state" \
  ...
  echo "  7) Gaming"
  echo "  8) Optional"
  echo "  9) Desktop Basic"
  echo " 10) Desktop Full"
  ...
  for pick in "${picks[@]}"; do
    case "$pick" in
      ...
      8) INSTALL_OPTIONAL=true ;;
      9) INSTALL_DESKTOP_BASIC=true ;;
      10) INSTALL_DESKTOP_FULL=true ;;
      *)
